...
def smart_imread(img_path, grayscale=False):
    ...
    def try_load(path):
        ...
        try:
            if grayscale: return np.array(Image.open(path).convert('L'))
            else: return np.array(Image.open(path).convert('RGB'))
        except: return None
...
def run_pipeline(args):
    ...
    if original_length < MIN_FRAMES:
        ...
        log(f"Padding: {original_length} -> {MIN_FRAMES}")
    ...
    # Use all frames as local window
    local_frames = original_length
